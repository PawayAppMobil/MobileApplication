    ...
    Column(modifier = Modifier
        ...
        Card(
            ...
            Column(
                ...
                Button(
                    onClick = { viewModel.createReport("12345678", reportType, startDate, endDate) },
        ...
        when (reportState) {
            is Resource.Success -> {
                ...
                if (reports.isNullOrEmpty()) {
                    Text(text = "No hay reportes disponibles", color = MaterialTheme.colorScheme.onBackground)
                } else {
                    Text(
                    ...
                    LazyColumn {
                        items(reports) { report ->
                            ReportItem(report) { reportId ->
                                viewModel.deleteReport("12345678", reportId)
                            }
                        }
                    }
                }
...
@Composable
fun ReportItem(report: Report, onDelete: (String) -> Unit) {
    Card(
        modifier = Modifier
            .fillMaxWidth()
